...
| **Weather Updates**   | Current Conditions | Check current temperature, humidity, wind, and AQI for any city worldwide using Open-Meteo.                                                                                              |
|                       | Smart Alerts       | Background workers poll extreme weather indicators (rain > 70%, UV > 8, AQI > 150) and automatically push WhatsApp warnings.                                                             |
|                       | Daily Briefings    | Set a custom `briefingHour` (e.g. 8 AM) to receive a witty AI-generated summary of your day's local forecast.                                                                            |
...
|                | `!topdf`     |                               | Convert a Word/Excel/PPT document or Image to a PDF. Reply to a document to use this command.                                                                         | Whitelist |
|                | `!compresspdf`| `[low\|medium\|high]`        | Compress a PDF document to reduce file size. Default is low. Reply to a PDF to use this command.                                                                      | Whitelist |
| **Reminders**  | `!remind`    | `<time> <msg>`                | Set a manual reminder using shorthand (`5m`, `2h`, `1d`) or full 5-part cron expressions (`0 */2 * * *`).                                                             | Whitelist |
|                | `!reminders` |                               | List all active scheduled reminders for the current chat.                                                                                                             | Whitelist |
|                | `!delremind` | `<id>`                        | Cancel and delete a specific reminder by its ID.                                                                                                                      | Whitelist |
| **AI**         | `!hana`      | `<query>`                     | Ask the AI a question or issue a command. The AI has direct access to run bot commands (like converting images, managing alerts) on your behalf! The AI can also handle **bulk file processing sessions** (e.g., combining 10 images into a PDF, bulk EXIF stripping, or bulk conversion). | Whitelist |
