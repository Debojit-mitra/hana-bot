...
import { initAlertStates, processResults, syncAlertStates, getAlertStates } from './alert.service.js';
...
async function runAlertCheck(): Promise<void> {
    ...
    try {
        ...
        for (const message of alertMessages) {
            const uptimeChannels = config.alerts.channels.filter(c => c.events.includes('uptime'));
...
const handleAlerts: CommandHandler = async (ctx) => {
    ...
    switch (subcommand) {
        ...
        case 'sub': {
            ...
            if (!eventsStr) {
                await ctx.reply('Usage: `!alerts sub <events>`\nEvents can be: `uptime`, `jenkins` (comma separated)\nExample: `!alerts sub uptime,jenkins`');
                return;
            }
            const requestedEvents = eventsStr.split(',').map(e => e.trim().toLowerCase());
            const validEvents = requestedEvents.filter(e => e === 'uptime' || e === 'jenkins') as ('uptime' | 'jenkins')[];
            
            if (validEvents.length === 0) {
                await ctx.reply('❌ Invalid events specified. Use `uptime` or `jenkins`.');
                return;
            }

            const channelIndex = config.alerts.channels.findIndex(c => c.jid === ctx.jid);
            if (channelIndex > -1) {
                // Update existing
                const existingEvents = new Set(config.alerts.channels[channelIndex].events);
                validEvents.forEach(e => existingEvents.add(e));
                config.alerts.channels[channelIndex].events = Array.from(existingEvents);
            } else {
                config.alerts.channels.push({ jid: ctx.jid, events: validEvents });
            }
            await updateConfig({ alerts: config.alerts });
            await ctx.react('✅');
            await ctx.reply(`🔔 Subscribed this chat to alerts: ${validEvents.join(', ')}`);
            break;
        }
        ...
        case 'status': {
            ...
            const lines = [...states.values()].map(s => {
                ...
                const lastCheck = s.lastChecked ? formatTimestamp(s.lastChecked) : 'never';
                return `${icon} *${s.name}*\n   Last check: ${lastCheck}\n   Failures: ${s.consecutiveFailures}`;
            });

            const channelsStr = config.alerts.channels.map(c => `- \`${c.jid.split('@')[0]}\`: ${c.events.join(', ')}`).join('\n') || 'None configured';
            ...
            await ctx.reply([
                ...
                '',
                '*Monitored Sites:*',
                ...(lines.length > 0 ? lines : ['No sites being monitored.']),
                '━━━━━━━━━━━━━━━━━━',
            ].join('\n'));
            break;
        }
        ...
        default:
            await ctx.reply(
                ...
                '  `!alerts sub <events>` — Subscribe this chat (uptime, jenkins)\n' +
