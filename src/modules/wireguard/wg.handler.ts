  ...
  if (subcommand === "help") {
    await ctx.reply(
      "🔒 *WireGuard VPN*\n" +
        ...
        "  - `!wg add` _<name>_ : Add new peer (Admin Only)\n" +
        "  - `!wg revoke` _<name>_ : Revoke a peer (Admin Only)\n" +
...
const handleWgAdd: CommandHandler = async (ctx) => {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
    await ctx.reply(
      "❌ Name must be alphanumeric (dashes and underscores allowed).",
    );
    return;
  }
...
const handleWgRevoke: CommandHandler = async (ctx) => {
  ...
  try {
    ...
    await ctx.react("✅");
    await ctx.reply(
      `🗑️ Peer *${name}* has been revoked and removed from WireGuard.`,
    );
  } catch (err: any) {
    await ctx.react("❌");
