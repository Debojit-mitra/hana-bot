...
const handleAddSite: CommandHandler = async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      "❌ Invalid URL. Make sure it includes the protocol (https://).",
    );
    return;
  }
  ...
  if (
    config.ping.sites.some((s) => s.name.toLowerCase() === name.toLowerCase())
  ) {
...
const handleRemoveSite: CommandHandler = async (ctx) => {
  ...
  const name = ctx.args[0];
  const config = getConfig();
  const idx = config.ping.sites.findIndex(
  ...
  await ctx.react("✅");
  await ctx.reply(
    `🗑️ Removed *${removed.name}* (${removed.url}) from monitoring.`,
  );
};
