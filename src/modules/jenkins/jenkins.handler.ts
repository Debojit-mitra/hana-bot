...
const handleJenkins: CommandHandler = async (ctx) => {
    ...
    switch (subcommand) {
        case 'on': {
            ...
            await ctx.react('✅');
            await ctx.reply(`🏗️ Jenkins integration *enabled*.\nListening on port ${envConfig.jenkins.port}.`);
            break;
        }
        ...
        case 'status': {
            const baseUrl = envConfig.jenkins.webhookBaseUrl ? envConfig.jenkins.webhookBaseUrl : `http://<bot-ip>:${envConfig.jenkins.port}`;
            ...
            if (envConfig.jenkins.serverUrl) {
                ...
                serverStatus = result.status === 'up' ? `✅ Online (${result.responseTime}ms)` : `❌ Offline (${result.error || 'Unknown Error'})`;
