...
export function startJenkinsServer(sock: WASocket): void {
    ...
    app.post('/webhook/jenkins', async (req, res) => {
        try {
            ...
            const status = payload.status || payload.build?.status || payload.build?.phase;
            ...
            else if (status.toLowerCase() === 'started' || status.toLowerCase() === 'running') icon = '🚀';
            ...
            const jenkinsChannels = config.alerts.channels?.filter(c => c.events.includes('jenkins')) || [];

            for (const channel of jenkinsChannels) {
                try {
                    await sock.sendMessage(channel.jid, { text: message });
                } catch (err) {
                    logger.error({ err }, 'Failed to send Jenkins alert message to channel');
                }
            }
    ...
    server = app.listen(envConfig.jenkins.port, () => {
        logger.info(`Jenkins webhook server listening on port ${envConfig.jenkins.port}`);
    });
}
